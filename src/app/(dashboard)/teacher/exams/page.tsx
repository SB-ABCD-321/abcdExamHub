...
export default async function TeacherExamsPage(props: { searchParams: Promise<{ page?: string }> }) {
    ...
    const [exams, totalCount] = await Promise.all([
        ...
        db.exam.count({ where: { workspaceId: { in: workspaceIds }, ...examFilter } })
    ]);
                ...
                {totalCount === 0 ? (
                    ...
                    <>
                        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
                            {exams.map((exam) => (
                                <Card key={exam.id} className="group relative border border-slate-200/60 dark:border-zinc-800/60 bg-white dark:bg-zinc-900/90 rounded-[1.5rem] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col">
                                    <CardHeader className="p-6 pb-4">
                                        ...
                                        <div className="mt-1">
                                            <CardTitle className="text-xl font-bold line-clamp-2 leading-snug group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors h-[3.5rem]">
                                                {exam.title}
                                            </CardTitle>
                                    ...
                                    <CardContent className="p-6 pt-0 space-y-6 flex-1 flex flex-col justify-end">
                                        <div>
