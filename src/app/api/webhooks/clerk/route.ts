...
export async function POST(req: Request) {
        ...
        try {
            await db.user.create({
                data: {
                    ...
                    lastName: last_name || '',
                    imageUrl: image_url || '',
                    // Assuming the first user might need to be a STUDENT by default. Super admin can assign roles later.
                }
            })
        } catch (error) {
    ...
    if (eventType === 'user.updated') {
        const { id, email_addresses, first_name, last_name, image_url } = evt.data
        const primaryEmail = email_addresses?.length > 0 ? email_addresses[0].email_address : undefined
